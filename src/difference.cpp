...
#include <iostream>
#include "track.h"
const int skipFrames = 100;
const double contourMinArea = 500.0;
const double contourMaxArea = 50000.0;
bool inSpawnRegionDiff(const cv::Point& center, const Spawns& spawns) {
...
void smoothMask(cv::Mat& maskImage) {
...
//     diffEngine = cv::bgsegm::createBackgroundSubtractorMOG(100, 3, 0.7); // requires contrib module "bgsegm"
    diffEngine = cv::createBackgroundSubtractorKNN(50, 400.0, false);
...
void DifferenceTracker::processFrame(cv::Mat& frame, const Spawns& spawns, std::vector<Contour>& contours) {
    ...
    cv::Mat maskImageCopy = maskImage.clone();
